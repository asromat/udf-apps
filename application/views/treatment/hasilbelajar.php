        ...
        <div class="card image-gallery-card direction-rtl">
            <div class="card-body">
                ...
                <center>
                    ...
                    <h4 class="text-success">Nilai Posttest : <?= $posttest->skor ?></h4>
                ...
                <table class="table mb-0 table-hover mb-3">
                    ...
                    <?php $no = 1; foreach ($menubelajar->result() as $key => $data) {; ?>
                        <tr>
                            ...
                            <td><?= $data->skor ?></td>
                            <td><?= $this->fhe->get2w("tb_menu_hasilbelajar","user_id",$this->session->id,"tema_id",$data->tema_id)->row("skor") ?></td>
                            <td><?= $this->fhe->progressTest($data->skor,$this->fhe->get2w("tb_menu_hasilbelajar","user_id",$this->session->id,"tema_id",$data->tema_id)->row("skor")) ?></td>
                        </tr>
                    <?php } ?>
                    </tbody>
                </table>
                ...
                <a href="<?= base_url("") ?>" class="btn btn-primary mt-3 w-100">Lanjut Belajar</a>
            </div>
        </div>
    </div>
</div>
